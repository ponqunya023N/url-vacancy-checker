        ...
        if rows:
            ...
            for i, row in enumerate(rows, 1):
                try:
                    ...
                    # 画像URL取得の強化（解析結果に基づき rep_room-madori-src を最優先）
                    img_url = ""
                    # 1. まず本命のクラス名で探す
                    ...
                    # 2. 見つからない場合は従来の候補で探す
                    if not img_elem:
                        img_elem = row.query_selector("div.item_image img, .rep_room-image img")

                    if img_elem:
                        src = img_elem.get_attribute("src")
                        # アイコン画像（icn_）やボタン画像（button）を誤認しないためのフィルタ
                        if src and "icn_" not in src and "button" not in src:
                            img_url = urllib.parse.urljoin("https://www.ur-net.go.jp", src)
                        else:
                            print(f"  [DEBUG] 部屋{i}({room_name}): 間取り図以外の画像をスキップしました ({src})")

                    print(f"  [DEBUG] 部屋{i}({room_name}): 取得完了 (家賃: {rent}, 画像: {img_url})")
