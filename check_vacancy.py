...
def judge_vacancy(browser, name: str, url: str) -> dict:  # [037]
  ...
  # [048]
        rows = page.query_selector_all("tbody.rep_room tr")  # [049]
          # [050]
        # ログには名前を出さず、プレフィックスのみ出力する  # [051]
        match = re.match(r'(【.*?】)', name)  # [052]
        prefix = match.group(1) if match else "【不明】"  # [053]
        # 検出件数のみまず出力  # [054]
        print(f"[{timestamp()}] [DEBUG] {prefix}***: {len(rows)}件検出")  # [055]
  # [056]
        if rows:  # [057]
            ...
            for i, row in enumerate(rows, 1):  # [059]
                try:  # [060]
                    ...
                    if not rent or rent == "不明": continue  # [064]
  # [065]
                    ...
                    room_name = room_name_elem.inner_text().strip() if room_name_elem else f"部屋{i}"  # [071]
  # [072]
                    # 画像取得  # [073]
                    img_url = ""  # [074]
  ...
  # [078]
                    if img_elem:  # [079]
                        ...
                        if src and "icn_" not in src and "button" not in src:  # [081]
                            # 【秘匿化】URLを直接書かずSecretsから取得するように変更  # [082]
                            base_url = os.getenv("BASE_URL", "")  # [082-1]
                            img_url = urllib.parse.urljoin(base_url, src)  # [082-2]
  # [083]
                    # 部屋名（建物名含む）の詳細データを作成  # [084]
                    result["details"].append({  # [085]
                        "room_hash": make_hash(room_name),   # [086]
                        "text": f"🏢 <b>{room_name}</b>\n家賃: {rent} (共益費: {common})",  # [087]
                        "img_url": img_url  # [088]
                    })  # [089]
                except Exception:  # [090]
                    # エラーログの隠蔽  # [091]
                    print(f"  [DEBUG] 部屋データ取得エラー（詳細は秘匿されています）")  # [092]
                    continue  # [093]
              # [094]
            if found_valid_room:  # [095]
                ...
                return result  # [097]
  # [098]
        if "ございません" in page.content() or page.query_selector(".err-box"):  # [099]
            result["status"] = "not_available"  # [100]
          # [101]
        return result  # [102]
    except Exception:  # [103]
        # エラーログの徹底的な隠蔽  # [104]
        print(f"[{timestamp()}] 通信エラー発生（対象URL等の詳細は秘匿されています）")  # [105]
...
def send_telegram(name: str, url: str, new_rooms_details: list) -> None:  # [111]
  ...
  # [121]
    try:  # [122]
        ...
        print("Telegram送信エラー（詳細は秘匿されています）")  # [138]
  # [139]
def main() -> None:  # [140]
  ...
  # [154]
    with sync_playwright() as p:  # [155]
        ...
        for name, url in targets_list:  # [157]
            match = re.match(r'(【.*?】)', name)  # [158]
            prefix = match.group(1) if match else "【不明】"  # [159]
            safe_key = f"{prefix}{make_hash(name)}"  # [160]
  # [161]
            res = judge_vacancy(browser, name, url)  # [162]
            s = res["status"]  # [163]
              # [164]
            # 【変更点】合意した入れ子構造（辞書形式）に合わせて読み込みを修正  # [165]
            prev_data = prev.get(safe_key, {})  # [166]
            if isinstance(prev_data, list):  # 旧バージョンのリスト形式が残っている場合  # [167]
                prev_notified_hashes = prev_data  # [168]
            else:  # 合意した辞書形式の場合  # [169]
                prev_notified_hashes = prev_data.get("notified_rooms", [])  # [170]
  # [171]
            if not isinstance(prev_notified_hashes, list):  # [172]
                prev_notified_hashes = []  # [173]
  # [174]
            # 現在見つかった部屋のハッシュリスト  # [175]
            current_rooms_hashes = [d["room_hash"] for d in res["details"]]  # [176]
              # [177]
            # ログにハッシュ値を表示（デバッグ用）  # [178]
            hash_log = ", ".join(current_rooms_hashes) if current_rooms_hashes else "なし"  # [179]
            print(f"[{timestamp()}] {safe_key}: {s} (現在ハッシュ: {hash_log} / 保存済み数: {len(prev_notified_hashes)}件)")  # [180]
  # [181]
            # 今回の更新履歴を計算  # [182]
            updated_history = list(set(prev_notified_hashes + current_rooms_hashes))  # [183]
  # [184]
            if s in ["error", "unknown"]:  # [185]
                # エラー時はこれまでの状態と履歴をそのまま引き継ぐ  # [186]
                next_status_data[safe_key] = {  # [187]
                    "current_status": prev_data.get("current_status", s) if isinstance(prev_data, dict) else s,  # [188]
                    "notified_rooms": prev_notified_hashes  # [189]
                }  # [190]
            else:  # [191]
                # 「available」または「not_available」時  # [192]
                new_rooms_details = [d for d in res["details"] if d["room_hash"] not in prev_notified_hashes]  # [193]
                if new_rooms_details:  # [194]
                    send_telegram(name, url, new_rooms_details)  # [195]
                  # [196]
                # 【重要】合意した通りのデータ構造で保存  # [197]
                next_status_data[safe_key] = {  # [198]
                    "current_status": s,  # [199]
                    "notified_rooms": updated_history  # [200]
                }  # [201]
  # [202]
        browser.close()  # [203]
      # [204]
    with open(STATUS_FILE, "w", encoding="utf-8") as f:  # [205]
        json.dump(next_status_data, f, ensure_ascii=False, indent=2)  # [206]
  # [207]
if __name__ == "__main__":  # [208]
    main()  # [209]
